...
const MCInstrDesc &RISCVInstrInfo::getBrCond(RISCVCC::CondCode CC, bool imm) const {
  switch (CC) {
  default:
...
unsigned RISCVInstrInfo::insertBranch(
  ...
  MachineInstr &CondMI =
      *BuildMI(&MBB, DL, getBrCond(CC, !Cond[2].isReg())).add(Cond[1]).add(Cond[2]).addMBB(TBB);
  if (BytesAdded)
