...
#undef CASE_VMXOR_VMANDN_VMOR_OPCODES
  ...
  case ISD::VECTOR_SHUFFLE: {
    if (Subtarget->hasExtXcvsimd())
    {
        ...
        if (VT == MVT::v2i16) {
            Opcode = RISCV::CV_SHUFFLE_SCI_H;
            imm |= (Shuffle->getMaskElt(1) & 1) << 1;
            imm |= Shuffle->getMaskElt(0) & 1;
        } else {
            switch (Shuffle->getMaskElt(3)) {
            default:
            case 0: Opcode = RISCV::CV_SHUFFLEI0_SCI_B; break;
            case 1: Opcode = RISCV::CV_SHUFFLEI1_SCI_B; break;
            case 2: Opcode = RISCV::CV_SHUFFLEI2_SCI_B; break;
            case 3: Opcode = RISCV::CV_SHUFFLEI3_SCI_B; break;
            }
        ...
        if (VT == MVT::v2i16) {
        Opcode = RISCV::CV_SHUFFLE2_H;
        imm |= (Shuffle->getMaskElt(1) & 1) << 16;
        imm |= Shuffle->getMaskElt(0) & 1;
        } else {
        Opcode = RISCV::CV_SHUFFLE2_B;
        imm |= (Shuffle->getMaskElt(3) & 3) << 24;
        imm |= (Shuffle->getMaskElt(2) & 3) << 16;
        imm |= (Shuffle->getMaskElt(1) & 3) << 8;
        imm |=  Shuffle->getMaskElt(0) & 3;
        }
        SDValue Imm = SDValue(selectImm(CurDAG, DL, MVT::i32, imm, *Subtarget), 0);
